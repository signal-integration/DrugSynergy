...
em = filtered_results[[1]][filtered_results[[1]]$prof_index == 14,]
...
dbs <- all_dbs[c(117, 113, 94, 93, 95, 99)]

joined = cluster_annotation_terms(deg, 'P', dbs, n = 40, p_cut = 0.05)

library(dplyr)
my_filter = joined$Adjusted.P.value < 0.05 & joined$ratio >= 0.1


joined_f = filter(joined, my_filter)


joined_f$split_var = 'b'
joined_f$split_var[joined_f$gene_set == 'all'] = 'a'
...
selected_terms = levels(joined_f$Term)[c(8, 9, 16, 17, 18, 28, 30, 37, 14)]
...
#rename terms
levels(joined_f_others_s$Term)[1] = "MHC I antigen presentation"
levels(joined_f_others_s$Term)[2] = "Cytokine signaling"
levels(joined_f_others_s$Term)[3] = "IFN signaling"
levels(joined_f_others_s$Term)[4] = "ISG15 antiviral mechanism"
levels(joined_f_others_s$Term)[5] = 'Metallothioneins'
levels(joined_f_others_s$Term)[6] = 'Mineral absorption'
levels(joined_f_others_s$Term)[7] = 'Pos reg of T cell profil'
levels(joined_f_others_s$Term)[8] = 'Pos reg of transcription'

levels(joined_f_others_s$Term)[9] = 'TNF signaling'
...
theme_bw() + facet_grid(.~split_var, scales = "free_x", space = "free_x", drop = TRUE)+theme(text = element_text(size=12, face = 'bold'), panel.border = element_blank())+theme(panel.spacing = unit(1, "lines"))+scale_y_discrete(limits = rev(levels(joined_f_others_s$Term)[c(8,2,3,4,1,9,7,6,5)]))
