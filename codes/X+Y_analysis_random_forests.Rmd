...
boxplot(simulated_values ~ design, ylab = 'simulated expression', col = 'gray', ylim  = c(1.5, 12))
...
features = extract_limma_features(my_data[,-(1:2)], design)

predicted_classes = predict(rf_model, newdata = features, type = 'prob')
...
max_match = t(apply(predicted_classes, 1, function(x) c(max(x), which.max(x))))
  ...
  optimal_match[features[,1]>0.05, "prof_index"] = 2
  optimal_match[features[,1]>0.05, "case"] = 1
  optimal_match[features[,1]>0.05, "outcome"] = 2
  optimal_match[features[,1]>0.05, "type"] = 'A'
  
  results_2.5h = optimal_match
...
my_data_filtered_matched = find_optimal_match(my_data_filtered[,-(1:2)], design, PROFCODES)

head(my_data_filtered_matched)

my_data_filtered_matched_2.5h = my_data_filtered_matched
save(my_data_filtered_matched_2.5h, file =  "my_data_filtered_matched_2.5h")
```
