...
samples = ncol(my_data[,-(1:2)])/4
design = factor(c(rep("CTRL", samples), rep("X", samples), rep("Y", samples), rep("YX", samples)))

results_1h = resolve_integration(data_1h, design, PROFCODES)
results_2.5h = resolve_integration(data_2.5h, design, PROFCODES)


save(results_2.5h, file = "results_2.5h")
```

```{r warning = FALSE, message = FALSE}

library(eulerr)
venn_df = stat_features[,c(18, 19, 21)]
venn_df[venn_df == -1] = 1
...
deg = my_results_1h[my_results_1h$prof_index != 2, ]
...
my_deg = my_results_1h[my_results_1h$prof_index != 2, ]
alluvial_cases = melt(table(my_deg$type, my_deg$case))
...
alluvial_cases$Var1 = factor(alluvial_cases$Var1, levels(alluvial_cases$Var1)[c(2,3,1)])
levels(alluvial_cases$Var1)[levels(alluvial_cases$Var1)=="P"] <- "syn"
levels(alluvial_cases$Var1)[levels(alluvial_cases$Var1)=="A"] <- "add"
levels(alluvial_cases$Var1)[levels(alluvial_cases$Var1)=="N"] <- "ant"

alluvial(alluvial_cases[,1:2], freq = alluvial_cases$value, col = col, cex = 0.1, alpha=0.6)
