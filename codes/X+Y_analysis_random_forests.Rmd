...
em = filtered_results[[1]][filtered_results[[1]]$prof_index == 27,]
...
joined = cluster_annotation_terms(deg, 'P', dbs, n = 10)


ggplot(joined, aes(x = gene_set, y = Term)) +
geom_point(aes(size = ratio, fill = score), shape = 21) +
scale_size_area(max_size = 8) + scale_fill_gradient(low = "gray70", high = "black") + theme_bw()

  
#filter terms
filtered_idx = c(1:3, 5:13, 15:17, 19:23)
...
ggplot(joined_f, aes(x = gene_set, y = Term))+
  geom_point(aes(size = ratio, fill = score), shape = 21)+ 
  scale_size_area(max_size = 10) + scale_fill_gradient(low = "gray70", high = "black") +
  theme_bw()
